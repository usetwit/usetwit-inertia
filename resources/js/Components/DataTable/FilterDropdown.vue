...
<template>
    <form @submit.prevent="$emit('apply')" class="flex flex-col">
        <FilterSelectOperator v-if="column.type !== 'boolean' && column.type !== 'active'" v-model="filter.operator"/>
        <template v-for="(constraint, i) in filter.constraints" :key="i">
            <FilterSelectMode v-if="column.type !== 'boolean' && column.type !== 'active'" :type="column.type"
            ...
            <FilterButtonRemove
                v-if="filter.constraints.length > 1 && column.type !== 'boolean' && column.type !== 'active'"
                v-model="filter.constraints"
                :index="i"
            />
        </template>
        <FilterButtonAdd v-if="column.type !== 'boolean' && column.type !== 'active' && filter.constraints.length < 5"
                         v-model="filter.constraints"
                         :type="column.type"
        />
        <FilterButtonClearApply v-model="filter.constraints" :type="column.type" @clear="$emit('filter')"
                                @apply="$emit('apply')"/>
    </form>
</template>
